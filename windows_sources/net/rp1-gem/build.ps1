<#
.SYNOPSIS  Build the RP1 Gigabit Ethernet (Cadence GEM) NDIS miniport for ARM64.
           NDIS 6.30 miniport: link ndis.lib, entry GsDriverEntry.
...
$Deliver = "C:\ai_project\AprWindowsDriver\windows_driver\net"
...
$Objs=@()
Write-Host "== Compiling ($Arch, NDIS 6.30) ==" -ForegroundColor Cyan
foreach ($s in $Sources) {
    ...
    $a = @("/nologo","/c","/W4","/O2","/Gy","/Zi","/kernel","/GS","/D_WIN64","/DNDEBUG") + $ArchDefs + @(
         "/I$IncKmCrt","/I$IncKm","/I$IncShared","/Fo$obj","/Fd$OutDir\rp1gem.pdb",$s)
    & $Cl @a
...
$Sys = Join-Path $OutDir "rp1gem.sys"
...
$la = @("/nologo","/DRIVER","/SUBSYSTEM:NATIVE,10.0","/MACHINE:$Machine","/NODEFAULTLIB","/INTEGRITYCHECK",
        "/ENTRY:GsDriverEntry","/DEBUG","/PDB:$OutDir\rp1gem.pdb","/LIBPATH:$LibKm",
        "ndis.lib","libcntpr.lib","BufferOverflowFastFailK.lib","ntoskrnl.lib","hal.lib",
        "/OUT:$Sys") + $Objs
& $Link @la
