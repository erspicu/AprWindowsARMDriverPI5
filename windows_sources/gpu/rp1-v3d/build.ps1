<#
.SYNOPSIS  Build the RP1/V3D WDDM render KMD scaffold for ARM64 (cl/link direct).
           DxgkInitialize is exported by displib.lib (same as the DOD).
.EXAMPLE   pwsh -File build.ps1 -CompileOnly   # scaffold compile check
           pwsh -File build.ps1                 # full build (needs ddi.c stubs)
#>
[CmdletBinding()]
param(
...
$Deliver = "C:\ai_project\AprWindowsDriver\windows_driver\gpu"
...
foreach ($s in $Sources) {
    ...
    $a = @("/nologo","/c","/W4","/O2","/Gy","/Zi","/kernel","/GS","/D_WIN64","/DNDEBUG") + $ArchDefs + @("/I$IncKmCrt","/I$IncKm","/I$IncShared","/Fo$obj","/Fd$OutDir\rp1v3d.pdb",$s)
    & $Cl @a
...
$Sys = Join-Path $OutDir "rp1v3d.sys"
Write-Host "== Linking ($Arch) ==" -ForegroundColor Cyan
$la = @("/nologo","/DRIVER","/SUBSYSTEM:NATIVE,10.0","/MACHINE:$Machine","/NODEFAULTLIB","/INTEGRITYCHECK","/ENTRY:GsDriverEntry","/DEBUG","/PDB:$OutDir\rp1v3d.pdb","/LIBPATH:$LibKm","displib.lib","libcntpr.lib","BufferOverflowFastFailK.lib","ntoskrnl.lib","hal.lib","/OUT:$Sys") + $Objs
& $Link @la
