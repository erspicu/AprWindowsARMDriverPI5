<# Build the BCM2712 GPIO GpioClx driver for ARM64 (KMDF + GpioClx). #>
[CmdletBinding()]
...
Write-Host "== Compiling ($Arch, KMDF + GpioClx) ==" -ForegroundColor Cyan
foreach($s in $Sources){
  $obj=Join-Path $OutDir ([IO.Path]::GetFileNameWithoutExtension($s)+".obj"); $Objs+=$obj
  $a=@("/nologo","/c","/W4","/O2","/Gy","/Zi","/kernel","/GS","/D_WIN64","/DNDEBUG","/DKMDF_VERSION_MAJOR=$KmdfMaj","/DKMDF_VERSION_MINOR=$KmdfMin")+$ArchDefs+@("/I$IncKmdf","/I$IncKmCrt","/I$IncKm","/I$IncShared","/Fo$obj","/Fd$OutDir\bcm2712gpio.pdb",$s)
...
$Sys=Join-Path $OutDir "bcm2712gpio.sys"
Write-Host "== Linking ==" -ForegroundColor Cyan
$la=@("/nologo","/DRIVER","/SUBSYSTEM:NATIVE,10.0","/MACHINE:$Machine","/NODEFAULTLIB","/INTEGRITYCHECK","/ENTRY:FxDriverEntry","/DEBUG","/PDB:$OutDir\bcm2712gpio.pdb","/LIBPATH:$LibKm","/LIBPATH:$LibKmdf","wdfdriverentry.lib","wdfldr.lib","msgpioclxstub.lib","libcntpr.lib","BufferOverflowFastFailK.lib","ntoskrnl.lib","hal.lib","/OUT:$Sys")+$Objs
...
Get-ChildItem $Deliver -Filter bcm2712gpio.sys | Select-Object Name,Length | Format-Table -AutoSize
