<#
.SYNOPSIS  Build the RP1 DesignWare I2C SpbCx controller driver for ARM64.
           KMDF + SpbCx: link wdfdriverentry/wdfldr + spbcxstubs (spb 1.1).
...
foreach ($s in $Sources) {
    ...
    $a = @("/nologo","/c","/W4","/O2","/Gy","/Zi","/kernel","/GS","/D_WIN64","/DNDEBUG",
           ...
           "/I$IncKmdf","/I$IncSpb","/I$IncKmCrt","/I$IncKm","/I$IncShared","/Fo$obj","/Fd$OutDir\rp1i2c.pdb",$s)
    & $Cl @a
...
$Sys = Join-Path $OutDir "rp1i2c.sys"
...
$la = @("/nologo","/DRIVER","/SUBSYSTEM:NATIVE,10.0","/MACHINE:$Machine","/NODEFAULTLIB","/INTEGRITYCHECK",
        "/ENTRY:FxDriverEntry","/DEBUG","/PDB:$OutDir\rp1i2c.pdb",
