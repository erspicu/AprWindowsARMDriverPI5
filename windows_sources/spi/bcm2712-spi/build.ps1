<#
.SYNOPSIS  Build the BCM2712 SPI SpbCx controller driver for ARM64.
...
foreach ($s in $Sources) {
    ...
    $a = @("/nologo","/c","/W4","/O2","/Gy","/Zi","/kernel","/GS","/D_WIN64","/DNDEBUG",
           ...
           "/I$IncKmdf","/I$IncSpb","/I$IncKmCrt","/I$IncKm","/I$IncShared","/Fo$obj","/Fd$OutDir\bcm2712spi.pdb",$s)
    & $Cl @a
...
$Sys = Join-Path $OutDir "bcm2712spi.sys"
...
$la = @("/nologo","/DRIVER","/SUBSYSTEM:NATIVE,10.0","/MACHINE:$Machine","/NODEFAULTLIB","/INTEGRITYCHECK",
        "/ENTRY:FxDriverEntry","/DEBUG","/PDB:$OutDir\bcm2712spi.pdb",
