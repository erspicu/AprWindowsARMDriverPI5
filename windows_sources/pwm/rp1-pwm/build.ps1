<#
.SYNOPSIS  Build the RP1 PWM KMDF driver for ARM64. (Bespoke KMDF template.)
...
$Deliver = "C:\ai_project\AprWindowsDriver\windows_driver\pwm"
...
foreach ($s in $Sources) {
    ...
    $a = @("/nologo","/c","/W4","/O2","/Gy","/Zi","/kernel","/GS","/D_WIN64","/DNDEBUG",
           ...
           "/I$IncKmdf","/I$IncKmCrt","/I$IncKm","/I$IncShared","/Fo$obj","/Fd$OutDir\rp1pwm.pdb",$s)
    & $Cl @a
...
$Sys = Join-Path $OutDir "rp1pwm.sys"
...
$la = @("/nologo","/DRIVER","/SUBSYSTEM:NATIVE,10.0","/MACHINE:$Machine","/NODEFAULTLIB","/INTEGRITYCHECK",
        "/ENTRY:FxDriverEntry","/DEBUG","/PDB:$OutDir\rp1pwm.pdb","/LIBPATH:$LibKm","/LIBPATH:$LibKmdf",
