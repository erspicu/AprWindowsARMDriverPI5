<#
.SYNOPSIS  Build the CYW43455 Wi-Fi NDIS miniport registration skeleton (ARM64).
...
$Deliver = "C:\ai_project\AprWindowsDriver\windows_driver\wifi"
...
$Objs=@()
Write-Host "== Compiling ($Arch, NDIS 6.30 WiFi skeleton) ==" -ForegroundColor Cyan
foreach ($s in $Sources) {
    ...
    $a = @("/nologo","/c","/W4","/O2","/Gy","/Zi","/kernel","/GS","/D_WIN64","/DNDEBUG") + $ArchDefs + @(
         "/I$IncKmCrt","/I$IncKm","/I$IncShared","/Fo$obj","/Fd$OutDir\cyw43455wifi.pdb",$s)
    & $Cl @a
...
$Sys = Join-Path $OutDir "cyw43455wifi.sys"
...
$la = @("/nologo","/DRIVER","/SUBSYSTEM:NATIVE,10.0","/MACHINE:$Machine","/NODEFAULTLIB","/INTEGRITYCHECK",
        "/ENTRY:GsDriverEntry","/DEBUG","/PDB:$OutDir\cyw43455wifi.pdb","/LIBPATH:$LibKm",
