<#
.SYNOPSIS  Build the RP1 CFE camera AVStream minidriver skeleton for ARM64.
           AVStream (ks.sys): link ks.lib + ksguid.lib, entry GsDriverEntry.
...
$Deliver = "C:\ai_project\AprWindowsDriver\windows_driver\camera"
...
$Sources = (@(Get-ChildItem $ProjDir -Filter *.c) + @(Get-ChildItem $ProjDir -Filter *.cpp)) | Select-Object -ExpandProperty FullName
$Objs=@()
Write-Host "== Compiling ($Arch, AVStream) ==" -ForegroundColor Cyan
foreach ($s in $Sources) {
    ...
    $a = @("/nologo","/c","/W4","/O2","/Gy","/Zi","/kernel","/GS","/D_WIN64","/DNDEBUG") + $ArchDefs + @(
         "/I$IncKmCrt","/I$IncKm","/I$IncShared","/Fo$obj","/Fd$OutDir\rp1cfe.pdb",$s)
    & $Cl @a
...
$Sys = Join-Path $OutDir "rp1cfe.sys"
...
$la = @("/nologo","/DRIVER","/SUBSYSTEM:NATIVE,10.0","/MACHINE:$Machine","/NODEFAULTLIB","/INTEGRITYCHECK",
        "/ENTRY:GsDriverEntry","/DEBUG","/PDB:$OutDir\rp1cfe.pdb","/LIBPATH:$LibKm",
        "ks.lib","ksguid.lib","libcntpr.lib","BufferOverflowFastFailK.lib","ntoskrnl.lib","hal.lib",
        "/OUT:$Sys") + $Objs
& $Link @la
