...
DefinitionBlock ("Dsdt.aml", "DSDT", 2, "RPIFDN", "RPI5    ", 2)
{
  Scope (\_SB_)
  {
    ...
    //
    // ===== AprWindowsDriver addition: V3D 7.1 GPU (BCM2712 VideoCore VII) =====
    // render-only WDDM KMD binds here (rp1v3d.sys, ACPI\RPIF000D). From Pi5 DT
    // bcm2712-ds.dtsi v3d@2000000 (brcm,2712-v3d): 3 MMIO blocks (hub/core0/sms)
    // at CPU 0x1002000000/0x1002008000/0x1002030800; GIC SPI 250/249
    // (ACPI GSIV = SPI+32 = 282/281). Non-coherent (_CCA 0). _STA off until the
    // GPU power domain is enabled — set to 0xF here to expose it to Windows.
    //
    Device (GPU0) {
      Name (_HID, "RPIF000D")
      Name (_UID, 0x0)
      ...
      Method (_CRS, 0, Serialized) {
        Name (RBUF, ResourceTemplate () {
          QWordMemory (ResourceConsumer, , MinFixed, MaxFixed, NonCacheable, ReadWrite,
            0x0, 0x0000001002000000, 0x0000001002003FFF, 0x0, 0x4000)   // hub
          QWordMemory (ResourceConsumer, , MinFixed, MaxFixed, NonCacheable, ReadWrite,
            0x0, 0x0000001002008000, 0x000000100200DFFF, 0x0, 0x6000)   // core0
          QWordMemory (ResourceConsumer, , MinFixed, MaxFixed, NonCacheable, ReadWrite,
            0x0, 0x0000001002030800, 0x0000001002030EFF, 0x0, 0x0700)   // sms
          Interrupt (ResourceConsumer, Level, ActiveHigh, Exclusive) { 282, 281 }
        })
        Return (RBUF)
      }
    }
