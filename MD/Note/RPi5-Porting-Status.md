...
| ~~Bluetooth（#12）~~ 🔵 | **H4 framing + BCM init x64 模擬 12/12** | UART I/O、.hcd 韌體；bthport 整合需 `bthx.h`（SDK 缺）|

### 🟡 需「大工程或缺件」（x64 可寫結構，完整功能仍需實機/額外專案）
| 項目 | 卡點 |
|------|------|
| GPU V3D（#13） | **UMD**（Mesa v3d 移植＝獨立大專案）＋ V3D MMU/CL 提交需實機 |
| WiFi（🟡 #6） | **dot11/WDI** 完整堆疊（極大）＋ SDIO function ＋ 韌體載入 |
...
| 12 | **Bluetooth（BCM43438）** 🔵 | KMDF (UART H4 HCI) | `windows_driver/bluetooth/btbcm.sys` | H4 HCI 框架（opcode LE/param/長度/邊界）+ BCM bring-up 序列；**x64 模擬 12/12 驗證** | UART WDFIOTARGET I/O、.hcd 韌體、bthport 整合需 `bthx.h`（此 WDK 缺）|
...
| # | 驅動 | 狀態 | 位置 | 餘下 |
|---|------|------|------|------|
| 5 | **WiFi（CYW43455）** | NDIS 註冊骨架（已 link，**缺 dot11/WDI 協定層**） | `windows_driver/wifi/cyw43455wifi.sys` | Native-802.11/WDI、SDIO function driver、韌體載入、scan/connect/OID（工程量極大）|
...
| 驅動 | Linux 來源 | Windows 框架 | 備註 |
|------|-----------|--------------|------|
| WiFi (CYW43455) | `brcmfmac` | NDIS 802.11/WDI | 🟡 NDIS 註冊骨架（見 🟡 #7）；dot11/WDI + SDIO + 韌體未完，工程量極大 |
| ~~Bluetooth (43438)~~ | `hci_bcm.c` | Windows BT | ✅ 見 🟢 #12（`btbcm.sys`）；H4+BCM 序列；bthport 整合需 bthx.h |
