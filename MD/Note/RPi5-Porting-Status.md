...
| 4 | **PCIe/RP1 bus（命脈）** 🟢→🔵 | UEFI ACPI（主線）/ KMDF bus driver（備案）| `uefi_fixed/Rp1.asi`(主線) ／ `windows_driver/pcie-rp1/rp1bus.sys`(備案,19KB) | **主線＝做法 A：UEFI ACPI 直接描述 RP1 子裝置**（`RPIF000n`，免 bus driver 即列舉，已 build 進 `RPI_EFI.fd`）。**備案＝做法 B：`rp1bus.sys`** 綁 `PCI\VEN_1DE4&DEV_0001`、map BAR1、WDFCHILDLIST 動態列舉子 PDO + 匯出 `GUID_RP1BUS_INTERFACE_STANDARD`（用於「不能改韌體」情境，Pi5 上不存在），ARM64 link 乾淨。決策見 `20260625-2230` | **PCIe RC 列舉**（UEFI ECAM）+ **MSI-X→RP1 內部 IC(0x108000,61 IRQ) demux**（兩法都需，GpioClx 角色）需實機 |
