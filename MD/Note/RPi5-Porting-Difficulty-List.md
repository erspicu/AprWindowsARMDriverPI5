...
| **Bluetooth（CYW43455 BT）** | **比 WiFi 低一個數量級**：HCI 標準協定、inbox 全包上層。**`bthx.h` 不在現代 WDK**（Win8 IHV 專屬，已移除；IOCTL hex 未公開）→ **改走 inbox `BthUart.sys`**：ACPI 掛 UART 下 + INF `Needs=BthUart.NT`，只需處理 BCM `.hcd` 韌體載入 + baud（薄 filter/vendor）。Phase A 邏輯 sim 35/35、Phase B WDF glue（UART/GPIO/bring-up）ARM64 /kernel 編譯過。詳見 [`bluetooth/`](bluetooth/) + `windows_sources/bluetooth/bcm43438/PLAN.md` | KMDF（BthUart INF + BCM 韌體 filter）| inbox BthUart/bthport |
